...
--The 'IS NOT' statement allows us to only view rows whose location is a nation/sovereign state
...
--Shows likelihood of dying from Covid in country quoted on line 15
...
GROUP BY location
ORDER BY Total_Death_Count desc
--Looks at all locations where continent is null, so that we are left with only the numbers
...
GROUP BY date
ORDER BY 1,2
--This shows us the cases and deaths that happened each day from 17/1/2020 until 21/10/2021 globally
...
FROM dbo.[Covid Deaths]
WHERE continent IS NOT null
--Current death percentage globally is 2.03%
...
FROM PopvsVac
--Either CONVERT or CAST can be used to change data type to integers
